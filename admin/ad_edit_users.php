...
<head>
    ...
    <title>Edit User | CineBook Admin</title>
    <!-- Google Icons -->
        ...
        <div class="content">
            ...
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                ...
                if (empty($errors)) {
                    if ($password !== '') {
                        // password changed
                        $pass_value = password_hash($password, PASSWORD_DEFAULT);
                        $stmt = $conn->prepare("UPDATE users SET name=?, username=?, email=?, contact_no=?, password=? WHERE id=?");
                        $stmt->bind_param('sssssi', $name, $username, $email, $contact_no, $pass_value, $id);
                    } else {
                        // keep old password
                        $stmt = $conn->prepare("UPDATE users SET name=?, username=?, email=?, contact_no=? WHERE id=?");
                        $stmt->bind_param('ssssi', $name, $username, $email, $contact_no, $id);
                    }

                    if ($stmt->execute()) {
                        echo "<script>alert('User updated successfully!'); window.location.href='ad_users.php';</script>";
                        exit;
                    } else {
            ...
            <div class="card">
                <form method="post" id="editUserForm">
                    ...
                    <input type="text" name="contact_no" id="contact_no" value="<?php echo htmlspecialchars($user['contact_no']); ?>">
